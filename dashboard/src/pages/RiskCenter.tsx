...
const RiskCenter: React.FC = () => {
  ...
  const [calcEntry, setCalcEntry] = useState(1.1000);
  const [calcStop, setCalcStop] = useState(1.0950);
  const [calcTarget, setCalcTarget] = useState(1.1100);
  ...
  const targetPerUnit = Math.abs(calcTarget - calcEntry);
  const potentialProfit = positionSize * targetPerUnit;
  const rrRatio = riskPerUnit > 0 ? targetPerUnit / riskPerUnit : 0;

  const limitPct = (current: number, max: number) => Math.min((current / max) * 100, 100);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatusCard
          label="Daily Loss"
          current={`${risk?.current_daily_loss_pct.toFixed(1)}%`}
          max={`${risk?.max_daily_loss_pct.toFixed(0)}%`}
          pct={risk ? limitPct(risk.current_daily_loss_pct, risk.max_daily_loss_pct) : 0}
          color="bg-rose-500"
          icon={<AlertTriangle size={20} className="text-rose-400" />}
        />
        <StatusCard
          label="Weekly Loss"
          current={`${risk?.current_weekly_loss_pct.toFixed(1)}%`}
          max={`${risk?.max_weekly_loss_pct.toFixed(0)}%`}
          pct={risk ? limitPct(risk.current_weekly_loss_pct, risk.max_weekly_loss_pct) : 0}
          color="bg-amber-500"
          icon={<AlertTriangle size={20} className="text-amber-400" />}
        />
        <StatusCard
          label="Open Positions"
          current={`${risk?.open_positions_count ?? 0}`}
          max={`${risk?.max_open_positions ?? 3}`}
          pct={risk ? limitPct(risk.open_positions_count, risk.max_open_positions) : 0}
          color="bg-cyan-500"
          icon={<Activity size={20} className="text-cyan-400" />}
        />
        <StatusCard
          label="Account Balance"
          current={`$${(risk?.account_balance ?? 10000).toLocaleString()}`}
          max="—"
          pct={0}
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
          ...
          <div className="space-y-5">
            <LimitBar
              ...
              value={`${risk?.max_risk_per_trade_pct.toFixed(1)}%`}
              desc="Maximum risk per single trade"
            />
            <LimitBar
              label="Daily Loss Limit"
              value={`${risk?.max_daily_loss_pct.toFixed(0)}%`}
              desc="Stop trading for the day after this loss"
            />
            <LimitBar
              label="Weekly Loss Limit"
              value={`${risk?.max_weekly_loss_pct.toFixed(0)}%`}
              desc="Stop trading for the week after this loss"
            />
            <LimitBar
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1 h-2 bg-slate-800 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${risk ? ((risk.current_daily_loss_pct / risk.max_daily_loss_pct) * 50).toFixed(0) : 0}%` }}
                  />
                </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        ...
        <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Inputs */}
          <div className="space-y-5">
            <CalcField label="Account Balance ($)" value={calcBalance} onChange={setCalcBalance} min={100} step={100} adornment="$" />
            <CalcField label="Risk Per Trade (%)" value={calcRiskPct} onChange={setCalcRiskPct} min={0.1} max={5} step={0.1} adornment="%" />
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-950 rounded-xl p-4 border border-slate-800 flex items-center justify-between">
              ...
              <span className={`text-2xl font-bold font-mono ${rrRatio >= 2 ? 'text-emerald-400' : rrRatio >= 1 ? 'text-amber-400' : 'text-rose-400'}`}>
                1:{rrRatio.toFixed(2)}
              </span>
            </div>
          </div>
...
const StatusCard = ({
  label, current, max, pct, color, icon
}: {
...
const LimitBar = ({ label, value, desc }: { label: string; value: string; desc: string }) => (
  <div>
...
const HealthIndicator = ({ label, value, status }: { label: string; value: string; status: 'good' | 'warning' | 'danger' }) => {
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={`px-2 py-0.5 rounded text-xs font-bold ${colors[status]}`}>{labels[status]}</span>
    </div>
  );
};

const CalcField = ({ label, value, onChange, min, max, step, adornment }: {
  label: string; value: number; onChange: (v: number) => void; min: number; max?: number; step: number; adornment?: string;
}) => (
  <div>
    <label className="text-sm text-slate-400 mb-1.5 block">{label}</label>
    <div className="relative">
      {adornment && (
        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500">{adornment}</span>
      )}
      <input
        type="number"
        value={value}
        ...
        className={`w-full bg-slate-950 border border-slate-800 rounded-lg py-2.5 focus:outline-none focus:border-emerald-500/50 transition-colors ${
          adornment ? 'pl-8 pr-4' : 'px-4'
        } text-sm font-mono`}
      />
    </div>
  </div>
